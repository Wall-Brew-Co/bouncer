  ...
  (st/spec
    {:type        :map
     :spec        (spec/keys :opt-un [::comment
                                      ::license
                                      ::plugins])
     :description "The rules that bouncer can enforce and their configuration."}))
...
(def default-config
  ...
  {:comment "The default configuration for Wall Brew projects."
   :rules   {:license {:comment  "https://github.com/Wall-Brew-Co/open-source?tab=readme-ov-file#licensing"
                       :disabled false}
